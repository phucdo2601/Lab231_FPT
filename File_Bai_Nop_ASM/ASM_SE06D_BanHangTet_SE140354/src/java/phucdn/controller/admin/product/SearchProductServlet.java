...
public class SearchProductServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            searchProduct = request.getParameter("txtProduct");
            ProductDAO proDAO = new ProductDAO();
            List<ProductDTO> result = null;
            if (searchProduct == null) {
                result = proDAO.findProductByProductID(searchProduct);
                request.setAttribute("SearchProductInfo", result);
                url = SUCCESS;
            } else {
                result = proDAO.findProductByProductID(searchProduct);
                if (result != null) {
                    request.setAttribute("SearchProductInfo", result);
                    request.setAttribute("txtProductSeach", searchProduct);
                    url = SUCCESS;
                }
            }
        } catch (Exception e) {
            log("ERROR at SearchProduct: " + e.getMessage());
        } finally {
